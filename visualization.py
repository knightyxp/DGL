...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    if is_master():
        logging.info('Total GPU memory occupied: {:.2f} GB'.format(total_mem))
        ...
        logging.info("This experiment is named {}\n".format(args.output_dir))
...
def train_epoch(epoch, args, model, train_dataloader, device, optimizer, global_step,
    ...
    if epoch == 0 and is_master():
        percentage,trainable_size_MB,total_param_size_MB  = model.module.print_trainable_params_percentage()
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % args.n_display == 0 and is_master():
                ...
                ##lr para for add parameters
                lr_tmp = optimizer.param_groups[2]['lr'] if args.optim == 'AdamW' else \
                            optimizer.get_lr()[2]               
...
def eval_epoch(model, test_dataloader, device, args=None):
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            if multi_sentence_:
                ...
                if len(filter_inds) > 0:
                    ...
                    ''''
                    if video is not None and video.size(0) != b:
                        video = video.repeat(video.size(0),1,1,1,1,1)
                    else:
                        video = video.repeat(b,1,1,1,1,1)
                    '''
                ...
                #global_on_frame_weights, cls_on_frame_weights, frame_prompt0_on_frame_weights  
                global_on_frame_weights = output['global_on_frame_weights']
                cls_on_frame_weights = output['cls_on_frame_weights']
                frame_prompt0_on_frame_weights =output['frame_prompt0_on_frame_weights']

                cls_save_path = './visualize/cls/'
                frame_prompt_save_path = './visualize/frame_prompt/'
                global_prompt_save_path = './visualize/global_prompt/'

                # Create directories if they do not exist
                os.makedirs(cls_save_path, exist_ok=True)
                os.makedirs(frame_prompt_save_path, exist_ok=True)
                os.makedirs(global_prompt_save_path, exist_ok=True)

                # Call the visualization function
                visualize_attention_weights(video, cls_on_frame_weights, index, cls_save_path, 'cls_token')
                visualize_attention_weights(video, frame_prompt0_on_frame_weights, index, frame_prompt_save_path, 'frame_prompt')
                visualize_attention_weights(video, global_on_frame_weights, index, global_prompt_save_path, 'global_prompt')
    ...
    #compute topk for visual
    #top5 = torch.topk(sim_matrix,5)[1]
...
def visualize_attention_weights(video_frames, attention_weights, index, save_path, weight_type):
    batch_size, _, num_frames, channels, height, width = video_frames.shape
    
    # Handle different types of attention weights
    if weight_type == "global_prompt":
        # Shape for global_prompt_on_frame_weight: [BZ, 4, 49, 12]
        _, num_prompts, num_patches, _ = attention_weights.shape
        # Reshape to the common format: [batch_size, num_frames, num_attention, num_patches]
        attention_weights = attention_weights.permute(0, 3, 1, 2)[:,:,0:1,:]
        
    elif weight_type in ["cls_token", "frame_prompt"]:
        # Shape for cls_token_on_frame_weight and frame_prompt_on_frame_weight: [B, 12, 1, 49]
        # Already in the common format
        _, num_frames, num_attention, num_patches = attention_weights.shape
    else:
        raise ValueError(f"Unsupported weight type: {weight_type}")
    
    downsample_factor = height // int((num_patches ** 0.5))
    attention_weight_h = int((num_patches ** 0.5))
    attention_weights = attention_weights.reshape(batch_size, num_frames, attention_weight_h, attention_weight_h)

    attention_weights_scaled = F.interpolate(attention_weights, 
                                             size=(height, width), 
                                             mode='bilinear', 
                                             align_corners=True)

    video_frames = video_frames.reshape(-1, num_frames, channels, height, width)

    selcted = [465,484,128,296,301,410,434,1,380,495,499,121]
    for batch_idx in range(batch_size):
        per_attention_weights_scaled = attention_weights_scaled[batch_idx]
        per_video_frames = video_frames[batch_idx]

        if index in selcted:
            frames_attention = []
            frames_original = []
            for frame_idx in range(num_frames):
                frame_attention_weights = per_attention_weights_scaled[frame_idx]
                frame_attention_weights = (frame_attention_weights - frame_attention_weights.min()) / (frame_attention_weights.max() - frame_attention_weights.min() + 1e-6)
                frame_attention_weights = frame_attention_weights.cpu().numpy()

                frame = per_video_frames[frame_idx].permute(1,2,0).cpu().numpy()
                ...
                frames_attention.append(frame_attention_weights)
                frames_original.append(frame)

            concatenated_attention = np.hstack(frames_attention)
            concatenated_frames = np.hstack(frames_original)

            fig, axes = plt.subplots(1, 1, figsize=(12, 1), dpi=300)
            axes.imshow(concatenated_frames)
            axes.imshow(concatenated_attention, cmap='jet', alpha=0.5)
            axes.get_xaxis().set_visible(False) 
            axes.get_yaxis().set_visible(False)
            plt.savefig('{}/ret_{}.png'.format(save_path, index), dpi=300) 
            plt.show()

            print('paint_index',index)        
            index+=1
